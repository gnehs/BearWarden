...
| 附件                            | 部分   | V14 本機 schema；V1／V2 與 sealed blob 均解密檔名並驗證 wrapped CEK，安全同步檔名、大小與 legacy 狀態；附件 metadata 以同步後的 server 最終狀態為準，wrapped key、短效 URL、路徑與 bytes 不進 renderer；clone 明確排除附件 | v2 下載、上傳、刪除、legacy Fix、進度／取消、額度與 UI |
