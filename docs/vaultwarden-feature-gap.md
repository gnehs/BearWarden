...
| 附件                            | 部分   | V14 本機 schema；V1／V2 與 sealed blob 均安全同步 metadata；下載時先驗證 fresh metadata，再取短效 URL bytes，並在主程序驗證 v2 HMAC 或解密 legacy AES、封鎖私網導向、原子 `0600` 寫入；金鑰、URL、路徑與 bytes 不進 renderer；鎖定會取消未落盤下載；記憶體內上限 128 MiB | 上傳、刪除、legacy Fix、大檔串流、進度／主動取消與額度提示 |
...
1. **資料安全與可復原性**：附件加密可攜、跨官方客戶端 fixture 與定期備份復原演練。
2. **個人保管庫完整度**：附件、TOTP 完整編輯、Passkey 與批次操作。
